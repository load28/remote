...
`reference-code/` 디렉토리에서 태그로 검색한다.
...
| 파일 | 내용 | 규칙 수 |
|------|------|---------|
| architecture.md | 의존성 방향, 레이어 분리, 결합도, 모듈 경계 | 10 |
| naming-conventions.md | PascalCase, camelCase, on/handle, Boolean 접두사 | 8 |
| component-patterns.md | SRP, 합성, 훅 규칙, key 활용, 클로저 트랩, 제거 예정 API 금지 | 15 |
| state-and-data.md | 함수형 setState, 파생 상태, Context, 비동기 | 16 |
| performance.md | 가상화, dynamic import, 워터폴, 의존성 배열 | 14 |
| testing-a11y.md | Testing Library, MSW, strict TS, 에러 바운더리, optional 남용 금지 | 15 |
...
- `reference-code/_tags.md`에서 하위 태그 선택
