...
> **핵심:** 먼저 레퍼런스 코드를 읽고, 그 패턴을 따라 코드를 작성한다.
> 규칙을 외워서 쓰는 것이 아니라, 검증된 코드를 복사/응용하여 작성한다.

#### Step 1: 복잡도에 맞는 레퍼런스 선택

[reference-code.md](reference-code.md)에서 작성할 코드의 복잡도에 맞는 레벨을 읽는다.

| 레벨 | 대상 | 레퍼런스 |
|------|------|---------|
| L1 | 단순 표시/인터랙션 컴포넌트 | 1-1, 1-2 |
| L2 | 상태관리 (필터, 페이지네이션) | 2-1, 2-2, 2-3 |
| L3 | 커스텀 훅 + API 호출 | 3-1, 3-2 |
| L4 | API 레이어 + 비즈니스 로직 | 4-1, 4-2, 4-3 |
| L5 | Context + Provider | 5-1, 5-2 |
| L6 | 합성 패턴 (Compound, Render Props) | 6-1, 6-2 |
| L7 | 성능 (가상화, lazy, transition) | 7-1, 7-2, 7-3, 7-4 |
| L8 | 에러 처리 + 테스트 | 8-1, 8-2, 8-3, 8-4 |
| L9 | 엔터프라이즈 (모듈 전체, Provider 조합) | 9-1, 9-2, 9-3, 9-4 |
| L10 | useEffect cleanup 총정리 | 10 |

#### Step 2: 상세 규칙 확인 (필요 시)
