...
| # | 확인 | 해당 시 조치 | 해당없음 조건 |
|---|------|------------|-------------|
| DG-1 | 테스트 피라미드 계층이 적절한가? [T-07] | 순수 함수/유틸리티는 unit, 컴포넌트+훅 조합은 integration, 핵심 사용자 흐름만 e2e | — (필수, 해당없음 불가. 항상 확인) |
| DG-2 | 버그 수정인 경우 재현 테스트를 먼저 작성하는가? [T-09] | 재현 테스트(fail) → 수정(pass) 순서 | 버그 수정이 아닌 새 기능 테스트 |
