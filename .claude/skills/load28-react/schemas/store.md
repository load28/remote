...
| # | 확인 | 해당 시 조치 | 해당없음 조건 |
|---|------|------------|-------------|
| DG-1 | 스토어가 외부 서비스/SDK에 직접 의존하는가? [A-03] | 인터페이스로 추상화, 구현체 주입 | 스토어가 순수 클라이언트 상태만 관리 |
| DG-2 | 컴포넌트에서 useStore() 전체 구독하고 있는가? | 필요한 필드만 selector로 선택, 여러 필드 시 useShallow | — (필수, 해당없음 불가. 항상 selector 사용) |
